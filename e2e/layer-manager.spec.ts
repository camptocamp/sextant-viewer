...
/**
 * Injects layers into the Pinia map store via the Vue app instance.
 * The app must already be loaded at this point.
 */
async function setMapLayers(page: Page, layers: GeoJsonLayer[]) {
  await page.evaluate(async (layers) => {
    ...
    await mapStore.setContext({
      layers,
      backgroundLayers: mapStore.context.backgroundLayers,
      view: mapStore.context.view,
    })
  }, layers)
}
...
test.describe('Layer Manager', () => {
  ...
  test('deletes a layer via the details panel', async ({ page }) => {
    ...
    await expect(page.getByText('Couche à supprimer')).not.toBeVisible()
  })
